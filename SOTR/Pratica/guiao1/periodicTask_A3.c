...
#define USE_CPU 0					// The cpu to be used for affinity
...
void * Thread_1_code(void *arg)
{
	...
	tp.tv_sec = periodicity/1000; //em s - PERIOD_S;	 //variavel global - para a periodicity
...
int main(int argc, char *argv[])
{
	...
	int priority = atoi(argv[2]); // (integer, [1,99])
	periodicity = atoi(argv[3]); //range [50,500] ms - variavel global
	...
	CPU_SET( USE_CPU, &cpuset); //set CPU 0 on cpuset
	...
	err=pthread_create(&threadid, &attr, Thread_1_code, &procname); //cria uma thread com estas caracteristicas - prioridade, paracidade
 	if(err != 0) {
...
void Heavy_Work(unsigned char FirstFlag)
{
	...
	lower=0;
	upper=100;
	subInterval= 500000; // mexe no took do computador - no tempo que demora a correr o Heavy_Work
